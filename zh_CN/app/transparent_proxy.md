...
以这篇教程为基础，写了一个用树莓派做透明翻墙网关的说明。使用v2ray内置的DNS功能和内置的geosite.dat，国内网站用国内的DNS服务器解析。名单外的网站走代理通道用8.8.8.8解析。网络流量也分流，国内网站和国内IP直接连接。其余全部走代理。属于白名单模式。制作说明网址：https://github.com/MassSmith/smgate/wiki
更多说一点：
透明代理一般都是运行在网关处，目前用的最多的是刷到路由器中。v2ray加密很强，相对来讲对运行设备性能要求高一点。所以一般的路由器比较难以胜任。而性能高的路由器，价格又很贵，保守估计可能在千元以上。而且刷路由器技术门槛也要高一些。
我参考这篇透明代理的教程，还有其他一些网友的使用经验，做了一个用树莓派做透明翻墙网关。其间，运行测试了将近一年，随着v2ray的不断完善，这个用树莓派做网关的方法，也不断完善，我个人在这将近1年的使用过和中，体验也越来越不错。目前来看，我觉得各方面都还算比较完善，就想把这个方法分享出来。供有这方面需要的网友参考。
个人觉得这个方法，优点有如下几点。一个是树莓派3b的性能足够，当然用3b+或4，更足够。另外，制作好一个系统镜像后（这是我做好的系统镜像备份：https://raw.githubusercontent.com/MassSmith/smgate/master/v2raygate.imz ），刷到一个新的树莓派中，就非常简单。直接写入到TF卡中，再把路由器的LAN口IP更改为：192.168.99.1。关闭路由器DHCP功能。取可。
